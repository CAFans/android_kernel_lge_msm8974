						...
						void *raw_data, int silent)
{
	...
	if (!err) {
        ...
        {
		/* FIXME
		 * root's gid should be AID_SDCARD_RW or
		 * multiuser_get_uid(userid, sb_info->options.sddfs_gid)?
		 */
		setup_derived_state(sb->s_root->d_inode,
			PERM_ROOT, sb_info->options.userid, AID_ROOT,
			multiuser_get_uid(0, sb_info->options.sdfs_gid), false);
            sb_info->obbpath_s = kzalloc(PATH_MAX, GFP_KERNEL);
#ifdef CONFIG_MACH_LGE
            if(sb_info->obbpath_s)
